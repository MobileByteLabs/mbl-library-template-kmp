...
# sync-dirs.sh
# Script to sync framework-related directories and files from upstream template repository
# This helps keep deployment configs, CI/CD, and tooling up-to-date with the template
...
# Default upstream URL - the template repository
DEFAULT_UPSTREAM_URL="https://github.com/MobileByteLabs/mbl-library-template-kmp.git"
...
# Directories and files to sync from upstream
# These are framework/tooling related - not library code
SYNC_DIRS=(
    "scripts"
    "config"
    ".github"
...
declare -A EXCLUSIONS=(
    [".github"]="workflows/template-customize.yml:file"
    ["root"]="customizer.sh:file sync-dirs.sh:file README.md:file settings.gradle.kts:file"
)
...
show_help() {
    ...
    echo "  This script syncs framework-related directories and files from the upstream"
    echo "  template repository. It preserves your library code and customized configs."
    echo
...
print_banner() {
    echo -e "${BLUE}╔════════════════════════════════════════════╗${NC}"
    echo -e "${BLUE}║${BOLD}     KMP Library Template Sync Tool         ${NC}${BLUE}║${NC}"
    echo -e "${BLUE}╚════════════════════════════════════════════╝${NC}"
    echo
}
...
cleanup_temp_dirs() {
    ...
    find . -type d -name "temp_*" -exec rm -rf {} + 2>/dev/null || true
    show_progress
}
...
if ! git rev-parse --verify "origin/main" >/dev/null 2>&1; then
    print_warning "main branch not found, using default branch: ${BOLD}$DEFAULT_BRANCH${NC}"
...
if [ "$DRY_RUN" = false ]; then
    ...
    if ! git diff --quiet --exit-code || ! git diff --quiet --exit-code --cached; then
        ...
        git add "${SYNC_DIRS[@]}" "${SYNC_FILES[@]}" 2>/dev/null || true
        git commit -m "sync: Update framework files from upstream template

This commit syncs the following items with upstream template:
- Directories: ${SYNC_DIRS[*]}
- Files: ${SYNC_FILES[*]}

Upstream: $DEFAULT_UPSTREAM_URL" || handle_error "Failed to commit changes"
        show_progress
